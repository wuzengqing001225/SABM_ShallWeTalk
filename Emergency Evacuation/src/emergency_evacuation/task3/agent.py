...
MENTAL_PROMPTS = {
    0: "selfish, not willing to help others, but full of confidence.",
    1: "selfish, not willing to help others, and afraid of difficulties.",
}
...
class Human(Agent):
    ...
    def get_attribute_prompt(self):
        prompt = f"{MENTAL_PROMPTS[self.capability]} {CAPABILITY_PROMPTS[self.mental]}"
        return prompt
    ...
    def form_current_state_stage1(self, persona, num_agents_around, dist_to_nearest_exit):
        ...
        prompt += dedent(f"""\
            ...
            You are {persona} Please tell me your feeling about the situation around you in one sentence showing if you are panic or not.
            """)
        return dedent(prompt)
...
You can choose from ['bottom', 'left', 'right']. Give your answer without any additional text.
    ...
    def form_action(self, target_exit, cur_pos, move_list, action_count):
        ...
        prompt += dedent(f"""\
            ...
            Select your move from these possible options (You can move in cardinal or intercardinal directions, options with obstacles or other people are excluded and not in the path, and option codes are in random order):
            {move_list}
